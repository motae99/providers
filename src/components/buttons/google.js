...
export default function () {
  ...
  const {connectGoogle, dbUser} = React.useContext(AuthContext);
  ...
  return (
    <View
      ...
      <TouchableNativeFeedback
        background={ripple}
        onPress={() => (dbUser?.google ? null : connectGoogle())}>
        <View style={{flexDirection: 'row', padding: Sizing.x10 + Sizing.x5}}>
          <Image
            ...
            source={
              dbUser?.google
                ? require('img/googleColor.png')
                : require('img/googleBlack.png')
            }
          />
          <Text style={{...Typography.body.x10, color: Colors.neutral.black}}>
            {dbUser?.google ? dbUser?.googleProfile.name : 'Connect Google'}
          </Text>
        </View>
      </TouchableNativeFeedback>
    </View>
